...
const CurrencyCalculationDetails = ({
  onCalculate,
  onReset,
  currencies,
  calculatedValue,
}) => {
  const { amountToConvert, rate, value, convertFrom, convertTo } =
    calculatedValue
    ...
    <Box>
      <Box>
        <Box
          ...
          fontSize={12}
          color="text.primary"
        >{`${amountToConvert} ${convertFrom.name} equals`}</Box>
        <Typography
          component="div"
          variant="h1"
          className={clsx(classes.textPrimary, classes.mbSpace)}
        >
