...
const SidebarOption = () => {
  ...
  const [showSidebarImage, setSidebarImageStatus] = useState(
    sidebarStyle.backgroundImage !== ''
  )
  ...
  const resetOption = () => {
    ...
    if (themeType === THEME_TYPES.SEMI_DARK) {
      ...
      resetColors = {
        ...
        navActiveBgColor: alpha(themeColors.primary, 0.1),
      }
    }
          ...
          <CmtGridView
            ...
            renderRow={(color, index) => (
              <Box key={index} position="relative">
                ...
                />
                {sidebarStyle.backgroundColor === color.backgroundColor && (
          ...
          <CmtGridView
            ...
            renderRow={(item, index) => (
              <Box
                ...
                position="relative"
                className="pointer"
                onClick={() => setSidebarImage(item)}
              >
                <CmtImage src={item.image} alt={item.name} />
                {sidebarStyle.backgroundImage === item.fullImage && (
                  <CheckCircleIcon className={classes.checkIcon} />
                )}
              </Box>
            )}
