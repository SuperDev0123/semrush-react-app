...
const ThemeStyles = () => {
  ...
  const { headerType, updateHeaderType, isSidebarFixed, eetSidebarPosition } =
    useContext(AppContext)
  const [isHeaderFixed, setHeaderStyle] = useState(
        ...
        <Box display="flex">
          <Box className="pointer" onClick={() => onFixHeaderClick()}>
            <Box position="relative">
              <CmtImage
                src={'/images/customizer/fix_header.png'}
                alt="Fix Header"
              />
              {isHeaderFixed && (
          ...
          <Box
            className="pointer"
            onClick={() => eetSidebarPosition(!isSidebarFixed)}
            ml={4}
          >
            <Box position="relative">
              <CmtImage
                src={'/images/customizer/fix_sidebar.png'}
                alt="Fix Sidebar"
              />
              {isSidebarFixed && (
                <CheckCircleIcon className={classes.checkIcon} />
