...
import {
  ...
  lightTheme,
  semiDarkTheme,
} from '../../../../common/config/theme/themeColors'

const AppContextProvider = ({ children }) => {
  const [theme, updateTheme] = useState(defaultContext.theme)
  const [drawerBreakPoint, updateDrawerBreakPoint] = useState(
    defaultContext.drawerBreakPoint
  )
  ...
  const [isSidebarFixed, eetSidebarPosition] = useState(
    defaultContext.isSidebarFixed
  )
  const [hasSideBarFilter, setHasSideBarFilter] = useState(
    defaultContext.hasSideBarFilter
  )
  const [activeStep, setActiveStep] = useState(defaultContext.activeStep)
  const [hasHeaderStepper, setHasHeaderStepper] = useState(
    defaultContext.hasHeaderStepper
  )
  ...
  const [showTourGuide, setTourGuideStatus] = useState(true)
  ...
  const getSidebarActiveColors = (option) => {
    ...
    if (themeType === THEME_TYPES.LIGHT) {
      return {
        ...
        navActiveBgColor: alpha(option.primary, 0.1),
      }
    }
  }
