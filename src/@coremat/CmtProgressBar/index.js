...
const CmtProgressBar = ({
  ...
  const valPositionTop = ['top-left', 'top-right', 'top-center'].some(
    (position) => position === valuePos
  )
  const valPositionBottom = [
    'bottom-left',
    'bottom-right',
    'bottom-center',
  ].some((position) => position === valuePos)
  const labelPositionTop = ['top-left', 'top-right', 'top-center'].some(
    (position) => position === labelPos
  )
  const labelPositionBottom = [
    'bottom-left',
    'bottom-right',
    'bottom-center',
  ].some((position) => position === labelPos)
      ...
      <Box
        ...
        className={clsx(classes.textContainer, 'Cmt-text-container', {
          [classes.textContainerRight]:
            valuePos === 'top-right' || valuePos === 'bottom-right',
          [classes.textContainerCenter]:
            valuePos === 'top-center' || valuePos === 'bottom-center',
        })}
      >
    ...
    return label ? (
      <Box
        ...
        className={clsx(classes.labelContainer, 'Cmt-label-container', {
          [classes.labelContainerRight]:
            labelPos === 'top-right' || labelPos === 'bottom-right',
          [classes.labelContainerCenter]:
            labelPos === 'top-center' || labelPos === 'bottom-center',
        })}
      >
        {label}
      </Box>
  ...
  return (
    <Box className={clsx(classes.root, 'CmtProgressBar-root')} {...restProps}>
      ...
      <Box className={classes.flexRoot}>
        ...
        {onlyContained ? (
          <Box
            className={clsx(classes.fillStyle, 'Cmt-fill-progress')}
            style={fillBarStyle}
          />
        ) : (
          <Box
