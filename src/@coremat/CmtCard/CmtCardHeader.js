...
const ActionsMenu = ({ actions, actionHandler, icon }) => {
  return (
    <CmtDropdownMenu
      ...
      items={actions}
      onItemClick={(value) => actionHandler(value)}
    />
  )
}
...
const CmtCardHeader = React.forwardRef(function CmtCardHeader(props, ref) {
  ...
  let showHideActionClass = showActions
    ? classes.showActionMenu
    : classes.hideActionMenu
  if (actionsPos === 'default') {
    showHideActionClass = showActions
      ? classes.showActionDefaultMenu
      : classes.hideActionDefaultMenu
  }
  ...
  return (
    <Box
      className={clsx(headerRootClasses, 'Cmt-header-root')}
    ...
    >
      ...
      {(actions.length > 0 || children) && (
        <Box
          ref={contentRef}
          className={clsx(menuRootActionsClasses, 'Cmt-action-default-menu')}
        >
          {children}
          {actions.length > 0 && (
