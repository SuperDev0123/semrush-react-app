...
const CmtBackDrop = ({
  ...
  const setHeight = () => {
    if (backLayerContentRef.current)
      getBackLayerContentHeight(backLayerContentRef.current.clientHeight)
    if (frontLayerRef.current)
      getFrontLayerHeight(frontLayerRef.current.clientHeight)
  }
  ...
  const backLayerComponent = () => {
    return (
      <Box className={clsx(classes.backLayer, 'Cmt-BackDrop-backLayer')}>
        <Box
          className={clsx(
            classes.backLayerHeader,
            'Cmt-BackDrop-backLayerHeader'
          )}
        >
          <Box
            flex={1}
        ...
        <PerfectScrollbar
          style={{ height: frontLayerHeight - headerSize - 20 }}
        >
  ...
  const frontLayerComponent = () => {
    return (
      <SlideHeight in={revealed} slidableHeight={expandableValue}>
        <Box
          ref={frontLayerRef}
          className={clsx(classes.frontLayer, 'Cmt-BackDrop-frontLayer')}
        >
          {subHeader && (
            <Box
              className={clsx(
                classes.frontLayerSubHeader,
                'Cmt-BackDrop-frontLayerSubHeader',
                { pointer: revealed }
              )}
...
CmtBackDrop.propTypes = {
  ...
  backLayerConcealed: PropTypes.oneOfType([
    PropTypes.element,
    PropTypes.string,
  ]),
