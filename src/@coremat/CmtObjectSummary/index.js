...
const CmtObjectSummary = ({
  ...
  if (typeof badge !== 'string') {
    const node = (
      <Box className={clsx(classes.badgeRoot, 'Cmt-badge')}>{badge}</Box>
    )
    ...
    return (
      <RenderBadge {...{ badge, anchorOrigin, badgeContent, classes }}>
        {typeof avatar === 'string' ? (
          <CmtAvatar src={avatar} alt={title} {...avatarProps} />
        ) : (
          avatar
        )}
      </RenderBadge>
    ...
    return (
      <Box className={clsx(classes.root, align)}>
        ...
        <Box className={clsx(classes.userInfo, 'Cmt-user-info')}>
          <Box
            component="p"
            className={clsx(classes.title, 'Cmt-title')}
            {...titleProps}
          >
            {title}
          </Box>
          <Box
            component="p"
            className={clsx(classes.subTitle, 'Cmt-sub-title')}
            {...subTitleProps}
          >
            {subTitle}
          </Box>
